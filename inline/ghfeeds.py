...
async def gh_feeds(jar):
    try:
        username = jar.text.split(maxsplit=1)[1]
    except IndexError:
        await jar.answer(
            [],
    ...
    for cont in data[:50]:
        ...
        article = await jar.builder.article(
            title=title,
            text=text,
            url=repo_url,
            parse_mode="html",
            ...
            buttons=[
                Button.url("View", url),
                Button.switch_inline("Search again", query=jar.text, same_peer=True),
            ],
        )
    ...
    await jar.answer(res, cache_time=5000, switch_pm=msg, switch_pm_param="start")
