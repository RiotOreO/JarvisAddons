...
async def jrvs(event):
    ...
    if jarr.endswith((".tgs")):
        ...
        cmd = ["lottie_convert.py", jarr, "jar.png"]
        file = "jar.png"
        ...
        stderr.decode().strip()
        stdout.decode().strip()
    elif jarr.endswith((".webp", ".png")):
        xx = await event.eor("`Processing`")
        im = Image.open(jarr)
        im.save("jar.png", format="PNG", optimize=True)
        file = "jar.png"
    else:
        xx = await event.eor("`Processing`")
        img = cv2.VideoCapture(jarr)
        heh, lol = img.read()
        cv2.imwrite("jar.png", lol)
        file = "jar.png"
